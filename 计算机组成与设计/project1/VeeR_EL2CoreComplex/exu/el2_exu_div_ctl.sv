...
//origin
   ...
   output logic          finish_dly_div_fp,                // Finish to match data
   ...
   logic [31:0]          out_fpadd;
   logic                 fpadd_done;
   logic [31:0]          out_fpmul;
   logic                 fpmul_done;
   logic [31:0]          out_fpdiv;
   logic                 fpdiv_done;
   ...
   logic                 stb_mul_d;
   logic                 stb_mul_dd;
   logic                 stb_mul_for_fp;
   logic                 stb_div;
   logic                 stb_div_d;
   logic                 stb_div_dd;
   logic                 stb_div_for_fp;

   logic [31:0]          divisor_d;
   logic [31:0]          divisor_dd;
   logic [31:0]          dividend_d;
   logic [31:0]          dividend_for_fp;
   logic [31:0]          divisor_for_fp;

   logic [3:0]           active_operation;

   assign finish_dly_div_fp = (active_operation[3] & finish_dly) |
                              (active_operation[2] & fpadd_done) |
                              (active_operation[1] & fpmul_done) |
                              (active_operation[0] & fpdiv_done);

   assign out[31:0] = ({32{active_operation[3] & finish_dly}} & out_raw[31:0]) |
                      ({32{active_operation[2] & fpadd_done}} & out_fpadd[31:0]) |
                      ({32{active_operation[1] & fpmul_done}} & out_fpmul[31:0]) |
                      ({32{active_operation[0] & fpdiv_done}} & out_fpdiv[31:0]);
   ...
   assign stb_mul = fp.fp_mul;
   assign stb_div = fp.fp_div;

   always @(posedge clk) begin
      stb_d       <= stb;
      stb_dd      <= stb_d;
      stb_mul_d   <= stb_mul;
      stb_mul_dd  <= stb_mul_d;
      stb_div_d   <= stb_div;
      stb_div_dd  <= stb_div_d;
      divisor_d   <= divisor;
   ...
   assign stb_mul_for_fp  = stb_mul | stb_mul_d | stb_mul_dd;
   assign stb_div_for_fp  = stb_div | stb_div_d | stb_div_dd;
   assign divisor_for_fp  = divisor | divisor_d | divisor_dd;
   assign dividend_for_fp = dividend | dividend_d;

   fp_adder simple_fp_adder (
        .input_a      ( dividend_for_fp ),
        .input_b      ( divisor_for_fp  ),
   ...
   multiplier simple_fp_multiplier (
        .input_a      ( dividend_for_fp ),
        .input_b      ( divisor_for_fp  ),
        .input_a_stb  ( stb_mul_for_fp  ),
        .input_b_stb  ( stb_mul_for_fp  ),
        .output_z_ack ( 1'b1            ),
        .clk          ( clk             ),
        .rst          ( ~rst_l          ),
        .output_z     ( out_fpmul       ),
        .output_z_stb ( fpmul_done      ),
        .input_a_ack  (                 ),
        .input_b_ack  (                 )
   );
   ...
   divider simple_fp_divider (
        .input_a      ( dividend_for_fp ),
        .input_b      ( divisor_for_fp  ),
        .input_a_stb  ( stb_div_for_fp  ),
        .input_b_stb  ( stb_div_for_fp  ),
        .output_z_ack ( 1'b1            ),
        .clk          ( clk             ),
        .rst          ( ~rst_l          ),
        .output_z     ( out_fpdiv       ),
        .output_z_stb ( fpdiv_done      ),
        .input_a_ack  (                 ),
        .input_b_ack  (                 )
   );
...
module fp_operation_latch (
    input wire clk,          // 时钟
    input wire dp_valid,     // 数据有效信号
    input wire fp_add,       // 浮点加法操作
    input wire fp_mul,       // 浮点乘法操作  
    input wire fp_div,       // 浮点除法操作
    output reg [3:0] a       // 4位输出信号
);

// 内部信号定义
wire add_only, mul_only, div_only;
wire unique_operation;
wire all_zero;

assign add_only = fp_add & ~fp_mul & ~fp_div;
assign mul_only = ~fp_add & fp_mul & ~fp_div; 
assign div_only = ~fp_add & ~fp_mul & fp_div;
assign unique_operation = add_only | mul_only | div_only;

assign all_zero = ~fp_add & ~fp_mul & ~fp_div;

always @(posedge clk) begin
    if (dp_valid) begin
        if (unique_operation) begin
            case ({fp_add, fp_mul, fp_div})
                3'b100: a <= 4'b0100; 
                3'b010: a <= 4'b0010; 
                3'b001: a <= 4'b0001; 
                default: a <= a;     
